...
def execute(cfg):
    ...
    #prediction
    train_pred = model.predict(X_train[:,18:])
    train_pred_prob = model.predict_proba(X_train[:,18:])

    test_pred = model.predict(X_test[:,18:])
    test_pred_prob = model.predict_proba(X_test[:,18:])

    val_pred = model.predict(X_val[:,18:])
    val_pred_prob = model.predict_proba(X_val[:,18:])
    ...
    print("MODEL Path: ", root_dir, models_dir, model_name, model_alg+model_id)
    
    print("MODEL Performance: ", performance)
    
    print("Output PSRS SCORE: ", cfg.output_best_param_dir)
